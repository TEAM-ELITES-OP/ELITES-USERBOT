...
async def admin_groups(SAVAGE):
    SAVAGEgroups = []
    async for dialog in SAVAGE.client.iter_dialogs():
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            SAVAGEfile = await event.client.download_media(SAVAGEresponse, "./temp")
            return await unzip(SAVAGEfile)
...
async def iphonex(text):
    ...
    eliteboy = r.get("message")
    SAVAGEurl = url(eliteboy)
    if not SAVAGEurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(eliteboy).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    SAVAGE = await sticcers[0].click("me", hide_via=True)
    if SAVAGE:
        await bot.send_file(int(chat_id), SAVAGE, reply_to=reply_to_id)
        await SAVAGE.delete()
