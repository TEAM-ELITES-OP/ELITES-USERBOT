...
async def _(event):
    ...
    SAVAGEevent = await edit_or_reply(event, "Downloading to my local, for analysis  🙇")
    if event.reply_to_msg_id:
        ...
        if (
            ...
            await SAVAGEevent.edit("Starting analysis, using IBM WatSon Speech To Text")
            headers = {
            ...
            if "results" in r:
                ...
                await SAVAGEevent.edit(string_to_show)
            else:
                await SAVAGEevent.edit(r["error"])
            # now, remove the temporary file
            os.remove(required_file_name)
    else:
        await SAVAGEevent.edit("Reply to a voice message, to get the relevant transcript.")
