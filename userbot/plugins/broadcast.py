...
async def SAVAGEbroadcast_send(event):
    if event.fwd_from:
        return
    SAVAGEinput_str = event.pattern_match.group(1)
    if not SAVAGEinput_str:
    ...
    SAVAGEevent = await edit_or_reply(
        event,
    ...
    await SAVAGEevent.edit(resultext)
    if BOTLOG:
...
async def SAVAGEbroadcast_add(event):
    if event.fwd_from:
        return
    SAVAGEinput_str = event.pattern_match.group(1)
    if not SAVAGEinput_str:
...
async def SAVAGEbroadcast_remove(event):
    if event.fwd_from:
        return
    SAVAGEinput_str = event.pattern_match.group(1)
    if not SAVAGEinput_str:
...
async def SAVAGEbroadcast_list(event):
    if event.fwd_from:
        return
    SAVAGEinput_str = event.pattern_match.group(1)
    if not SAVAGEinput_str:
    ...
    await edit_or_reply(SAVAGEevent, finaloutput)
...
async def SAVAGEbroadcast_delete(event):
    if event.fwd_from:
        return
    SAVAGEinput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(SAVAGEinput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {SAVAGEinput_str}",
            parse_mode=parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(SAVAGEinput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {SAVAGEinput_str}",
            parse_mode=parse_pre,
        )
    except Exception as e:
        await edit_delete(
