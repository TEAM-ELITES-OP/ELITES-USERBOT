...
HIMANSHU = str(ALIVE_NAME) if ALIVE_NAME else "elites User"
papa = borg.uid
...
async def gban(userbot):
    ...
    if not sender.id == hum.id:
        elitesbot = await edit_or_reply(ids, "Trying to gban this retard!")
    else:
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await elitesbot.edit(f"Gbaning This retard🚶\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await elitesbot.edit(f"`Either reply to a user or gib me user id/name`")
    try:        
        if gmute(user.id) is False:
            return await elitesbot.edit(f"**Error! User already gbanned.**")
    except:
        pass
    return await elitesbot.edit(
...
async def gunban(userbot):
    ...
    hum = await ids.client.get_me()
    if not sender.id == hum.id:
        elitesbot = await edit_or_reply(ids, "`Trying to ungban this kid...`")
    else:
        elitesbot = await edit_or_reply(ids, "`Ungban in progress...`")
    hum = await userbot.client.get_me()
    await elitesbot.edit(f"`Trying to ungban this kiddo...`")
    my_mention = "[{}](tg://user?id={})".format(hum.first_name, hum.id)
    f"@{hum.username}" if hum.username else my_mention
    await userbot.get_chat()
    a = b = 0
    ...
    if user:
        ...
        for i in testuserbot:
            try:
                ...
                a += 1
                await elitesbot.edit(f"Ok! Now Ungbaning this kiddo.\n\n**Please Wait Few Minutes**😏")
            except:
                b += 1
    else:
        await elitesbot.edit("**Reply to a user**")     
    try:
        if ungmute(user.id) is False:
            return await elitesbot.edit("**Error! I think User already ungbanned.**")
    except:
        pass
    return await elitesbot.edit(
...
async def handler(eliteboy): 
   if eliteboy.user_joined or eliteboy.user_added:      
       try:       	
         from userbot.plugins.sql_helper.gmute_sql import is_gmuted
         guser = await eliteboy.get_user()      
       ...
       if gmuted:
        for i in gmuted:
            if i.sender == str(guser.id):                                                                         
                chat = await eliteboy.get_chat()
                admin = chat.admin_rights
                creator = chat.creator   
                if admin or creator:
                 try:
                    await client.edit_permissions(eliteboy.chat_id, guser.id, view_messages=False)                              
                    await eliteboy.reply(
                     f"⚠️⚠️**Warning**⚠️⚠️\n\n`Gbanned User Joined the chat!!`\n"                      
                     f"**⚜️ Victim Id ⚜️**:\n[{guser.id}](tg://user?id={guser.id})\n"                   
                     f"**🔥 Action 🔥**  :\n`Banned this piece of shit....` **AGAIN!**")                                                
                 except:       
                    eliteboy.reply("`Shit!! No permission to ban users.\n@admins ban this retard.\nGlobally Banned User And A Potential Spammer`\n**Make your group a safe place by cleaning this shit**")                   
                    return
