...
@SAVAGEbot.on(admin_cmd(pattern=r"unpack", outgoing=True))
@SAVAGEbot.on(sudo_cmd(pattern=r"unpack"))
async def _(event):
...
@SAVAGEbot.on(admin_cmd(pattern="repack ?(.*)", outgoing=True))
@SAVAGEbot.on(sudo_cmd(pattern="repack ?(.*)", allow_sudo=True))
async def _(event):
