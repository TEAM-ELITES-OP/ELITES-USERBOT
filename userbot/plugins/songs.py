...
from SAVAGEbot.utils import admin_cmd, sudo_cmd, edit_or_reply, progress
...
async def nope(h1m4n5hu0p):
    SAVAGE = h1m4n5hu0p.pattern_match.group(1)
    if not SAVAGE:
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(SAVAGE))}")
